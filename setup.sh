...
if [ "$(uname)" == "Darwin" ]; then
    ...
    if ! brew info ffmpeg &>/dev/null; then
        ...
        echo "upgrading ffmpeg via Homebrew"
        brew upgrade ffmpeg
    fi
    ...
    if [ $(dpkg-query -W -f='${Status}' libav-tools 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
        echo "installing libav-tools via apt-get"
